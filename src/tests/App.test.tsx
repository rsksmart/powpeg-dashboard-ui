...
test('renders content', async () => {
  ...
  expect(await screen.findByTestId('btc-rsk-addresses-table')).toContainHTML(
    '<td>0x020ace...adae1c</td>'
  );
  expect(await screen.findByTestId('btc-rsk-addresses-table')).toContainHTML(
    '<td>0x0305a9...f14ed2</td>'
  );
  expect(await screen.findByTestId('btc-rsk-addresses-table')).toContainHTML(
    `<td>${mockedAddress.slice(0,8)}...${mockedAddress.slice(36,42)}</td>`
  );
});
...
test('click on address redirects to explorer', async () => {
  render(<App />);
  const link = await screen.findByText(`${mockedAddress.slice(0,8)}...${mockedAddress.slice(36,42)}`);
